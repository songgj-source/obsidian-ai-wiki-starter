...
사용 가능한 명령 키워드 및 자연어 매핑:

- `save`: 현재 작업 맥락을 저장한다. (트리거: "이번 작업 내용 옵시디언에 저장해줘", "옵시디언에 기록해줘")
- `ingest`: raw 자료를 wiki 자료로 가공한다. (트리거: "자료 분석해서 위키에 넣어줘", "문서 요약해서 옵시디언에 추가해줘")
- `query`: 기존 wiki와 log를 참조한다. (트리거: "옵시디언 참조해줘", "위키에서 찾아줘")
- `lint`: vault 구조와 규칙 위반을 점검한다. (트리거: "옵시디언 위키 점검해줘", "위키 정리해줘")
